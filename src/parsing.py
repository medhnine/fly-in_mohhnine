...
class Parse:
    ...
    def valid_meta(self, meta_d, info, nb_line):
        ...
        if len(meta_d) <= 2:
            raise ValueError(f"error in line {nb_line}: unvlaid meta data")
        ...
        for meta in result:
            res = meta.split('=')
            if res[0] in ['zone', 'color', 'max_drones'] and len(res) == 2:
                if res[0] == "zone":
                    ...
                    if info["color"] is not None:
                            raise ValueError(f"error in line {nb_line}: duplicated color in meta data"
)
                    info["color"] = res[1]
            else:
                raise ValueError(
    ...
    def hub_manger(self, graph : Graph, data : str, nb_line):
        ...
        if graph.nb_drones is None:
            raise ValueError(f"error in line {nb_line}: valid nb_drones is required as first line info")
        result : list[str] = data.split(':', 1)
        if len(result) != 2 or (result[0] != "start_hub" and result[0] != "hub" and result[0] != "end_hub"):
            raise ValueError(f"error in line {nb_line}: hub not valid")
        result[1] = result[1].strip()
        values = result[1].split(' ')
        if len(values) < 3:
            raise ValueError(f"error in line {nb_line}: hub info must at least have: hub x y")
        if values[0] in self.zones_name:
            raise ValueError(f"error in line {nb_line}: duplicated zone name")
        ...
        try:
            ...
            raise ValueError(f"error in line {nb_line}: x, y cordinate not valid")
        if len(values) > 3:
            store = ''
            ...
            if '[' in data[3]:
                ...
                raise ValueError(f"error in line {nb_line}: unvlaid metadata")
        else:
            info["zone_type"] = None
            info["color"] = None
            info["max_drones"] = None
    ...
    def connection_handler(self, data, count, graph: Graph, nb_line):
        if count < 2:
            raise ValueError(f"error in line {nb_line}: unvalid structure can not make connection add hubs")
        result = data.split(':', 1)
        if len(result) != 2 or result[1] == "":
            raise ValueError(f"error in line {nb_line}: error unvalid connection")
        ...
        if zone_name1 == zone_name2 or zone_name1 not in self.zones_name or zone_name2 not in self.zones_name:
            raise ValueError(f"error in line {nb_line}: unvalid connection same connection or zone are not known")
        ...
        if len(connection) <= 2:
            ...
            if len(meta) == 2:
                if '[' in meta[1]:
                    max_link_capacity = self.connection_meta(meta[1], nb_line)
                    graph.connect(zone_name1, zone_name2, max_link_capacity)
                else:
                    raise ValueError(f"error in line {nb_line}: error in connectin meta data")
            else:
    ...
    def connection_meta(self, meta_d, nb_line):
        data = meta_d[1:len(meta_d) - 1]
        if not data:
            raise ValueError(f"error in line {nb_line}: unvlid metadata/empty")
        result = data.split('=')
        if len(result) != 2 or result[0] != "max_link_capacity" or result[1] == "":
            raise ValueError(f"error in line {nb_line}: unvlaid meta data for connection")
        try:
            value = int(result[1])
        except ValueError as e:
            raise ValueError((f"error in line {nb_line}: enter valid value in metadata {e}"))
        if value <= 0:
            raise ValueError(f"error in line {nb_line}: unvlaid max_link_capacity number for connection")
        return value
        ...
        for line in filtered_data:
            if line.startswith("nb_drones:"):
                if graph.nb_drones is not None:
                    raise ValueError(f"error in line {nb_line}: duplicated nb_drones")
                ...
                hub = Zone(hub_info["name"], hub_info["x"], hub_info["y"], hub_info["color"], hub_info["zone_type"], float("inf"))
                hub.set_default()
                ...
                self.connection_handler(line, count, graph, nb_line)
            else:
    ...
    def handle_drones(self, data : str, graph : Graph, line : int) -> Graph:
        result = data.split(':', 1)
        if len(result) != 2:
            raise ValueError(
